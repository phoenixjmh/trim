...
RangeSliderChangeType RangeSliderBehaviorU(const ImRect& frame_bb, ImGuiID id, uint32_t* v1, uint32_t* v2, uint32_t v_min, uint32_t v_max, int power,  ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiStyle& style = g.Style;
...
// ~95% common code with ImGui::SliderFloat
RangeSliderChangeType RangeSliderUInt(const char* label, uint32_t* v1, uint32_t* v2, uint32_t v_min, int32_t v_max, const char* display_format, float power)
{
    RangeSliderChangeType change_type = RangeSliderChangeType::NONE;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    // Actual slider behavior + render grab
    change_type = RangeSliderBehaviorU(frame_bb, id, v1, v2, v_min, v_max, power, decimal_precision);
