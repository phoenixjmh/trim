...
bool ReadFrameFromOpenStream(int timestamp, AVStreamInfo &stream_info,
...
#define SKIP_FRAME()                                                           \
  ...
  response =
      av_seek_frame(stream_info.pFormatContext, stream_info.video_stream_index,
  ...
  while (av_read_frame(stream_info.pFormatContext, pPacket) >= 0) {
    ...
    SwsContext *sws_context = sws_getContext(
        pFrame->width, pFrame->height, stream_info.pCodecContext->pix_fmt,
        pFrame->width, pFrame->height, AV_PIX_FMT_RGB0, SWS_BILINEAR, NULL,
        NULL, NULL);
