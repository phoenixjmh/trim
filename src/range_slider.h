...
namespace ImGui
{
    enum class RangeSliderChangeType
    {
        NONE,
        FIRST_VALUE_CHANGED,
    ...
    float SliderCalcRatioFromValueInt(ImGuiDataType data_type, uint32_t v, uint32_t v_min, uint32_t v_max, float power, float linear_zero_pos)
    {
        return ImGui::ScaleRatioFromValueT<unsigned int, int, float>(data_type, v, v_min, v_max, false, power, linear_zero_pos);
    }

    // ~80% common code with ImGui::SliderBehavior
    //Overload for unsigned type
    bool RangeSliderBehaviorU(const ImRect& frame_bb, ImGuiID id, uint32_t* v1, uint32_t* v2, uint32_t v_min, uint32_t v_max, int power, ImGuiSliderFlags flags, RangeSliderChangeType& change_type)
    {
        ...
        change_type = RangeSliderChangeType::NONE;
        ...
        if (g.ActiveId == id)
        {
            if (g.IO.MouseDown[0])
            {
                ...
                // First determine which handle we're going to modify
                bool use_first_handle = fabsf(*v1 - new_value) < fabsf(*v2 - new_value);

                // Then only check if that specific handle's value would change
                if ((use_first_handle && *v1 != new_value) ||
                    (!use_first_handle && *v2 != new_value))
                {
                    if (use_first_handle)
                    {
                        *v1 = new_value;
                        change_type = RangeSliderChangeType::FIRST_VALUE_CHANGED;
                    }
                    else
                    {
                        *v2 = new_value;
                        change_type = RangeSliderChangeType::SECOND_VALUE_CHANGED;
                    }
                    value_changed = true;
                }
            }
     ...
     // Calculate slider grab positioning
        float grab_t = SliderCalcRatioFromValueInt(ImGuiDataType_U32, *v1, v_min, v_max, power, linear_zero_pos);
        ...
        // Calculate slider grab positioning
        grab_t = SliderCalcRatioFromValueInt(ImGuiDataType_Float, *v2, v_min, v_max, power, linear_zero_pos);
    ...
    bool RangeSliderUInt(const char* label, uint32_t* v1, uint32_t* v2, uint32_t v_min, int32_t v_max, const char* display_format, float power, RangeSliderChangeType& change_type, int time_numerator,int time_denominator)
    {
        bool value_changed = false;
        change_type = RangeSliderChangeType::NONE;
        ImGuiWindow* window = GetCurrentWindow();
        ...
        value_changed = RangeSliderBehaviorU(frame_bb, id, v1, v2, v_min, v_max, power, decimal_precision, change_type);
...
//TODO: use our actual functions here.
        ...
        double v2_seconds =
            *v2 * (double)time_numerator/ (double)time_denominator;

        double time_diff_seconds= v2_seconds-v1_seconds;
        std::string human_time_diff = GetHumanTimeString(time_diff_seconds);
    ...
    RangeSliderChangeType RangeSliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v1, float* v2, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
    {
        ImGuiContext& g = *GImGui;
        ImGuiWindow* window = GetCurrentWindow();
        const ImGuiStyle& style = g.Style;
        ...
        // Process clicking on the slider
        RangeSliderChangeType change_type = RangeSliderChangeType::NONE;
        bool value_changed = false;
        if (g.ActiveId == id)
        {
            if (g.IO.MouseDown[0])
    ...
    // ~95% common code with ImGui::SliderFloat
    RangeSliderChangeType RangeSliderFloat(const char* label, float* v1, float* v2, float v_min, float v_max, const char* display_format, float power)
    {
        RangeSliderChangeType change_type = RangeSliderChangeType::NONE;
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
        ...
        if (start_text_input || (g.ActiveId == id && g.TempInputId == id))
        {
            char fmt[64];
            snprintf(fmt, 64, "%%.%df", decimal_precision);
            if (TempInputScalar(frame_bb, id, label, ImGuiDataType_Float, v1, fmt))
                return change_type;
        }
        ...
        // Actual slider behavior + render grab
        change_type = RangeSliderBehavior(frame_bb, id, v1, v2, v_min, v_max, power, decimal_precision, 0);
